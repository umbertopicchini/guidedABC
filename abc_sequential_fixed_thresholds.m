function ABCdraws = abc_sequential_fixed_thresholds(problem,data,covariates,parmask,parbase,ABCthreshold_vector,summ_weights,numparticles,sampling,attempt,nu,type_copula,type_margin,nu_marg,folder)
...
ABCthreshold = ABCthreshold_vector(1);
...
save(sprintf('%s/evaltime_stage%d_attempt%d_copula%d_marg%d_nu%d_numarg%1.1f_part%d.txt',folder,t,attempt,type_copula,type_margin,nu,nu_marg,numparticles),'eval_time','-ascii')  
...
switch sampling
    ...
    case 'blockedopt'
        ...
        ABCthreshold_new = ABCthreshold_vector(t+1);
...
while t< length(ABCthreshold_vector) % keep doing as long as acceptance rate requirement is satsified
    t = t+1;
    ...
    sampling;
    ABCthreshold = ABCthreshold_vector(t);
           ...
           if distance < ABCthreshold
              ...
              switch sampling
               ...
               case 'blocked'
                    ...
                    % weights(success) = prior/mvnpdf(theta,proposal_mean,proposal_cov); % weights of accepted particles     prior /dens;%
                     if strcmp(sampling_temp,'hybrid')
                         distance_accepted(success) = distance; 
                     end
               case 'blockedopt'
                     distance_accepted(success) = distance; 
    ...
    case 'blocked'
        if t==2 && strcmp(sampling_temp,'hybrid')  % here we are in the hybrid case so we construct the blockedopt sampler for next iteration t>2
           %::: PREPARING FOR THE HYBRID STRATEGY ::::::::::::::::::::::
            if t+1 > length(ABCthreshold_vector)
                return
            end
            mean_theta = ABCdraws * normweights';  % weighted mean across all accepted parameters
            mean_simsum = simsumm_accepted * normweights'; % weighted mean across all accepted summaries
